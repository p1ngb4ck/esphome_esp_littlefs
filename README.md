...
# Custom Block Device Support

This fork adds support for custom block devices, allowing LittleFS to be used with any storage backend (FRAM, external SPI flash, etc.) instead of just ESP32 partitions or SD cards.

## Enabling Custom Block Device Support

Enable via `menuconfig`:
```
Component config -> LittleFS -> Custom block device support
```

Or via CMake:
```cmake
set(LITTLEFS_CUSTOM_BLOCK_DEVICE ON)
```

## Using Custom Block Devices

When custom block device support is enabled, the internal `lfs.h` header is exposed, giving you access to:
- `lfs_t` - LittleFS filesystem structure
- `lfs_config` - Configuration with block device callbacks
- `lfs_file_t`, `lfs_dir_t` - File/directory handles

Example:
```c
#include "lfs.h"

// Your block device callbacks
int my_read(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, void *buffer, lfs_size_t size) {
    // Read from your storage device
    return LFS_ERR_OK;
}

int my_prog(const struct lfs_config *c, lfs_block_t block, lfs_off_t off, const void *buffer, lfs_size_t size) {
    // Write to your storage device
    return LFS_ERR_OK;
}

int my_erase(const struct lfs_config *c, lfs_block_t block) {
    // Erase block (or no-op for FRAM/MRAM)
    return LFS_ERR_OK;
}
...
You can then register this filesystem with ESP-IDF's VFS to use standard POSIX file operations.
